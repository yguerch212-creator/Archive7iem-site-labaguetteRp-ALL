...
const medicalSoldbuchRoutes = require('./routes/medical-soldbuch.routes')
...
app.use('/api/medical-soldbuch', medicalSoldbuchRoutes)
...
app.get('/api/stats/archives', async (req, res) => {
  try {
    ...
    const rows = await query(`
      ...
      UNION ALL
      (SELECT 'piece', CAST(p.id AS CHAR), CONCAT(p.type, ' — ', p.titre), p.redige_par_nom, CAST(p.date_irl AS CHAR), p.created_at FROM affaires_pieces p)
      UNION ALL
...
app.get('/api/stats/pending', auth, async (req, res) => {
  try {
    const docs = await queryOne("SELECT COUNT(*) as c FROM documentation WHERE statut = 'en_attente'")
    ...
    // Incidents non traités (pour Feldgendarmerie)
