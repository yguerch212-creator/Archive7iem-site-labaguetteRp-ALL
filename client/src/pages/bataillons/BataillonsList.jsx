...
export default function BataillonsList() {
  ...
  return (
    <div style={{ padding: 'var(--space-lg)' }}>
      ...
      {bdm && (
        <div style={{ background: 'rgba(161,124,71,0.15)', border: '2px solid #a17c47', borderRadius: 8, padding: 'var(--space-md)', marginBottom: 'var(--space-lg)', textAlign: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-md)' }}>
        {bataillons.map(b => (
          <div key={b.id} onClick={() => b.isMember ? setSelected(b.id) : null} style={{
            background: '#f5f2e8', border: '1px solid #d4cbb8', borderRadius: 8,
...
function BataillonDetail({ id, onBack, user, reload }) {
  ...
  return (
    <div style={{ padding: 'var(--space-lg)' }}>
      <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#4a6741', cursor: 'pointer', fontSize: '0.9rem', marginBottom: 'var(--space-md)' }}>← Retour</button>
                ...
                <div style={{ marginTop: 8 }}>
                  {o.taches.map(t => (
                    <div key={t.id} onClick={() => o.statut !== 'annule' && toggleTache(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', cursor: o.statut === 'annule' ? 'default' : 'pointer', fontSize: '0.85rem' }}>
                      <span style={{ width: 18, height: 18, border: '2px solid #d4cbb8', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', background: t.completed ? '#4a6741' : 'transparent', color: '#fff', fontSize: '0.7rem', flexShrink: 0 }}>
                        {t.completed ? '✓' : ''}
                      </span>
      ...
      <div style={{ marginBottom: 8 }}>
        ...
        {form.taches.map((t, i) => (
          <div key={i} style={{ display: 'flex', gap: 6, marginTop: 4 }}>
            ...
            {form.taches.length > 1 && <button onClick={() => removeTache(i)} style={{ background: 'none', border: 'none', color: '#8b4a47', cursor: 'pointer' }}>✕</button>}
          </div>
        ))}
        <button onClick={addTache} style={{ background: 'none', border: 'none', color: '#4a6741', cursor: 'pointer', fontSize: '0.8rem', marginTop: 4 }}>+ Ajouter une tache</button>
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
...
function MembreForm({ bataillonId, onAdded }) {
  ...
  return (
    <div style={{ background: '#f5f2e8', border: '1px solid #d4cbb8', borderRadius: 8, padding: 'var(--space-md)', marginBottom: 'var(--space-md)' }}>
      ...
      {results.map(e => (
        <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: '1px solid #d4cbb8' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            ...
            <button onClick={() => add(e.id, 'officier')} className="btn btn-sm" style={{ fontSize: '0.7rem', background: '#a17c47' }}>Officier</button>
          </div>
        </div>
      ))}
...
function BdmForm({ bataillons, onSet }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="btn btn-sm" style={{ marginBottom: 'var(--space-md)', background: '#a17c47', color: '#fff' }}>Decerner Bataillon du Mois</button>
